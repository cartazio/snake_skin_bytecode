...
from .transfer import annotates, annotates_family
from .builtin_lattices import TypeLattice, SimpleType


def register_builtin_transfers(lattice: TypeLattice) -> None:
    """
    Register built-in transfer functions.
    
    Call this once with your lattice instance to enable
    default type propagation.
    """
    # Transfer functions close over the lattice
    _register_loads(lattice)
    _register_stores(lattice)
    _register_binary(lattice)
    _register_unary(lattice)
    _register_calls(lattice)
    _register_control(lattice)
    _register_build(lattice)
    _register_3_12_plus(lattice)
    _register_3_13_plus(lattice)
    _register_3_14_plus(lattice)


def _register_loads(lattice: TypeLattice):
    ...
    @annotates('LOAD_FAST', 'LOAD_FAST_CHECK', 'LOAD_FAST_BORROW')
...
def _register_binary(lattice: TypeLattice):
    ...
    @annotates('COMPARE_OP', 'IS_OP', 'CONTAINS_OP')
...
def _register_unary(lattice: TypeLattice):
    """Register unary operation transfer functions."""
    
    @annotates_family('UNARY_')
...
def _register_calls(lattice: TypeLattice):
    """Register call-related transfer functions."""
    
    @annotates('CALL', 'CALL_FUNCTION', 'CALL_METHOD')
...
def _register_build(lattice: TypeLattice):
    """Register structure building transfer functions."""
    
    @annotates('BUILD_LIST', 'LIST_EXTEND')
    ...
    @annotates('BUILD_SET', 'SET_UPDATE')
    ...
    @annotates('BUILD_MAP', 'BUILD_CONST_KEY_MAP', 'DICT_UPDATE')
...
def _register_3_13_plus(lattice: TypeLattice):
    """Register transfer functions for Python 3.13+ opcodes."""

    @annotates('LOAD_FAST_LOAD_FAST', 'LOAD_FAST_BORROW_LOAD_FAST_BORROW')
    ...
    @annotates('STORE_FAST_STORE_FAST')
    ...
    @annotates('STORE_FAST_LOAD_FAST')
    ...
    @annotates('SET_FUNCTION_ATTRIBUTE')
...
def _register_3_14_plus(lattice: TypeLattice):
    """Register transfer functions for Python 3.14+ opcodes."""

    @annotates('LOAD_SMALL_INT')
    ...
    @annotates('LOAD_COMMON_CONSTANT')
